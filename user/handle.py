...
def getAllRecordsFromDatabase(databaseName):
    ...
    print(connection)
    sql_select_Query = "select * from transaction group by card_number, timestamp order by card_number, timestamp desc"
...
def saveExtendedDataset(databaseName, extendedDataset):
    ...
    cursor = connection.cursor()
    cursor.execute("USE " + databaseName)
    file = open("SQL insert.txt", "r")
    sqlInsertScript = file.read()
...
def getTransacTionBaseNumberProperties(databaseName, cardNumber, timestamp):
    connection = getConnection(databaseName)
    cursor = connection.cursor()
    sqlQuery = f"select amount, timestamp from {databaseName}.transaction where card_number = {cardNumber} and timestamp <= {timestamp}  order by timestamp desc"
